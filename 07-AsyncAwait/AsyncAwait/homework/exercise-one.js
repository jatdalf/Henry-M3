...
async function problemB () {
  ...
  // AsyncAwait version
  //Opción 1 siempre consologuean en el mismo orden
  ...
  //Opción 2 -> creando otra función asíncrona:
  // async function readFileAsync(filename){
  //   blue( await promisifiedReadFile(filename))
  // }

  // readFileAsync('poem-one/stanza-02.txt') //sale del stack para resolverse
  // readFileAsync('poem-one/stanza-03.txt') //sale del stack para resolverse

  //Opción 3 -> Promise.all
   async function readFileAsync(filename){
    blue( await promisifiedReadFile(filename))
  }

  await Promise.all([readFileAsync('poem-one/stanza-02.txt'), readFileAsync('poem-one/stanza-03.txt')])
...
async function problemE () {
  ...
  try {
    ...
    // blue(stanza2)
    // const stanza3 = await promisifiedReadFile('poem-one/stanza-04.txt')
    // blue(stanza3)

    //Opción 2: Promise.all
    const stanzas = await Promise.all([promisifiedReadFile('poem-one/stanza-03.txt'), promisifiedReadFile('poem-one/stanza-04.txt')])
