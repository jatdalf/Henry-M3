...
async function problemB () {
  ...
  const promises = filenames.map(file => readFileAsync(file))
  //['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'...]
  //en promises nos queda guardado un arreglo de promesas
...
async function problemC () {
  ...
  //promisifiedReadFile('poem-two/stanza-01.txt','poem-two/stanza-02.txt'...)
  const stanzasPromises = await Promise.all(promises)

  stanzasPromises.forEach(stanza => blue(stanza))
  console.log('done');
...
async function problemD () {
  ...
  try {
    const promises = filenames.map(file => promisifiedReadFile(file))
    //promises es un arreglo de promesas
    const stanzasPromises = await Promise.all(promises)
    //stanzasPromises tiene un array de stanzas
    stanzasPromises.forEach(stanza => blue(stanza))
    console.log('done');
  } catch (error) {
    magenta(error)
